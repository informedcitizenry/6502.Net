...
namespace Core6502DotNet.Json
{
    ...
    /// </summary>
    public class ValidationError : JsonSerializable
    {
        /// <summary>
        ...
        public ValidationError(string message, Schema schema, string keyword, string instanceLocation)
        {
            Error = message;
            Schema = schema;
            KeywordLocation = $"{schema.GetPath(false)}/{keyword}";
            var absoluteUri = schema.GetPath(true);
        ...
        {
            var messageFrament = Error.Length > 39 ? Error.Substring(0, 40) : Error;
    ...
    /// </summary>
    public class AnnotationCollection : JsonSerializable
    {
        /// <summary>
        ...
        public void AddError(string message, Schema schema, string keywordLocation, JToken token)
            => Errors.Add(new ValidationError(message, schema, keywordLocation, token.FullPath()));
