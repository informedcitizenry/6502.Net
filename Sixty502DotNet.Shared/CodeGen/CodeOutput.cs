...
/// </summary>
public sealed class CodeOutput
{
    ...
    public void SetPC(int value)
    {
        if (AllowLongOutput)
        {
            int valueBank = (value & 0xff0000) / 0x10000;
            if (valueBank > CurrentBank)
            {
                CurrentBank = valueBank;
                while (_blocks < CurrentBank)
                    ResizeBuffer();
            }
        }
    ...
    public void SetLogicalPC(int value)
    {
        if (AllowLongOutput)
        {
            ...
            value = (int)((uint)value & 0xff00_ffff);
        }
    ...
    /// of the bytes added to the compilation.</returns>
    public void Add(string s) => Add(s, System.Text.Encoding.UTF8);

    /// <summary>
    /// Add the bytes for a string to the compilation.
    /// </summary>
    /// <param name="s">The string to add.</param>
    /// <param name="encoding">The <see cref="System.Text.Encoding"/> class to encode the output</param>
    /// <returns>A <see cref="List&lt;byte&gt;"/> of the bytes added to the compilation.</returns>
    public void Add(string s, System.Text.Encoding encoding) => AddBytes(encoding.GetBytes(s));
    ...
    public static int GetEffectiveAddress(int programCounter, int offset, int maxValue)
    {
        if (offset > maxValue)
        {
            offset -= (maxValue + 1) * 2;
    ...
    public override string ToString() => $"Start:    ${ProgramStart:X4}\nEnd:      ${ProgramEnd:X4}";
    ...
    /// <summary>
    /// Get or set the output format as described by the
    /// <see cref="IOutputFormatProvider"/>.
    /// </summary>
    public IOutputFormatProvider? OutputFormat { get; set; }

    /// <summary>
    /// Gets the flag indicating the binary output can allow >64KiB output and
    /// increase the output binary.
    /// </summary>
    public bool AllowLongOutput { get; set; }
