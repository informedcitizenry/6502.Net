...
public sealed class Evaluator : SyntaxParserBaseVisitor<ValueBase>
{
	...
	public ValueBase Eval(SyntaxParser.ExprContext expr, bool allowConstantAssignment = false)
	{
		...
		if (val.ValueType == ValueType.Integer && (val.AsDouble() < int.MinValue || val.AsDouble() > uint.MaxValue))
		{
            throw new Error(expr, "Illegal quantity");
		}
		val.Expression = expr;
        return val;
    }
    ...
    public override ValueBase VisitExpressionDictionary([NotNull] SyntaxParser.ExpressionDictionaryContext context)
    {
        ...
        {
			ValueBase key;
			if (members[i].identifierPart() != null)
			{
				key = new StringValue($"\"{members[i].identifierPart().NamePart()}\"");
			}
			else
			{
				key = EvalConstant(members[i].expr()[0]);
				if (!key.IsDefined)
				{
					throw new Error(members[i].expr()[0], "Key must be a constant expression");
				}
			}
            ...
            if (!dict.TryAdd(key, val, out DictionaryValue.AddStatus why))
            {
                throw why switch
                {
                    ...
                    DictionaryValue.AddStatus.KeyTypeInvalid	=> new Error(members[i].expr()[0], "Invalid key type"),
                    DictionaryValue.AddStatus.KeyTypeMismatch	=> new Error(members[i].expr()[0], "Key type mismatch"),
