...
{
    /// <summary>
    /// Responsible for formatting binary output as a .T64 formatted tape image.
    /// </summary>
    public class T64FormatProvider : IOutputFormatProvider
    {
        const string Header = "C64S tape image file\r\n";

        static byte[] GetNameBytes(string file, int size)
        {
            if (file.Length > size)
        ...
        public T64FormatProvider(string formatName)
