...
namespace UnitTest6502.Net
{
    ...
    public class PseudoOpsTest
    {
        ...
        TestController test_;
        PseudoOps6502 pseudo_;
        ...
        [Test]
        public void TestAsmValues()
        {
            SourceLine line = new SourceLine("test", 1, "mylabel .rta 0");
            line.Parse(s => s.Equals(".rta"), s => s.Equals("mylabel"));

            Assert.AreEqual(".rta", line.Instruction);

            pseudo_.AssembleLine(line);

            Assert.IsFalse(test_.Log.HasErrors);

            var compare = new byte[] { 0xff, 0xff };

            Assert.IsTrue(test_.Output.GetCompilation().SequenceEqual(compare));
